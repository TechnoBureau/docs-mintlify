---
title: Bruno v3 - The Next Evolution in Git-Friendly API Testing
description: Explore Bruno v3, the next evolution in Git-friendly API testing. Discover its new YAML-based OpenCollection format, enhanced Git integration, CI/CD benefits, and practical examples for requests, scripts, environments, and variables. A powerful open-source alternative for modern API workflows.
...
Bruno isn't just another API client—it's built for real-world workflows. The v3 release brings a fresh modern interface with better colors, spacing, and responsiveness that feels polished and intuitive.
...
Environments let you swap variables effortlessly between dev, staging, and production. In the v3  you can store environment files as YAML (or JSON) — YAML is recommended for readability and consistency with OpenCollection.
...
Bruno v3  is your ticket to a smoother, more collaborative API testing experience. With YAML beta shining bright and full release on the horizon, there's never been a better time to try it.

**Download the v3 here**: [Bruno v3 Release](https://www.usebruno.com/v3-release) (Windows, macOS, Linux available).
